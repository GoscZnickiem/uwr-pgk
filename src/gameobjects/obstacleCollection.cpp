...
ObstacleCollection::ObstacleCollection(long unsigned seed, int size, float boardSize, glm::vec3 boardCenter, float gridSize)
	: m_boardSize(boardSize),
	m_boardCenter(boardCenter),
	m_gridSize(gridSize),
	m_vis(AppData::data().modelObstacleV, AppData::data().modelObstacleI, AppData::data().shaderInstanced) {
...
void ObstacleCollection::render(float time) {
	...
	m_vis.m_shader->setUniform("boardSize", m_boardSize);
	m_vis.m_shader->setUniform("boardCenter", m_boardCenter);
	m_vis.render();
}
